...
class GoMonitor:
    ...
    async def _poll_bounties(self):
        ...
        proxy_manager = getattr(self._bot, "_network_manager", None)
        proxy_url = proxy_manager.get_proxy() if proxy_manager else None
            ...
            async with self._session.get(self._api_url, params=params, proxy=proxy_url, headers=headers, timeout=10) as response:
                if response.status == 530:
                    ...
                    if proxy_manager and proxy_url:
                        proxy_manager.report_result(proxy_url, False, 530)
                    return
                
                if response.status != 200:
                    logger.error(f"Failed to fetch bounties: HTTP {response.status}")
                    if proxy_manager and proxy_url:
                        proxy_manager.report_result(proxy_url, False, response.status)
                    return
                
                if proxy_manager and proxy_url:
                    proxy_manager.report_result(proxy_url, True, response.status)
    ...
    async def _process_bounty(self, bounty: Dict[str, Any]):
        ...
        if reward_sol >= self._reward_threshold and token_mint:
            ...
            mcap_usd = float(meta.get("market_cap_sol", 0)) * getattr(self._bot._telegram, "_sol_price", 150.0)
