    ...
    async def get_token_metadata(self, mint: str) -> Dict:
        ...
        try:
            async with self._session.get(url) as resp:
                if resp.status == 200:
                    data = await resp.json()
                    data.setdefault("market_cap_sol", 0)
                    data.setdefault("liquidity_sol", 0)
                    return data
            ...
            async with self._session.get(jup_url) as resp:
                if resp.status == 200:
                    ...
                    if token_info:
                        ...
                        # Convert USD price to market_cap_sol using estimated SOL price of 150
                        # Pump.fun tokens usually have 1B supply. 
                        # mc_sol = (price_usd * 1,000,000,000) / 150
